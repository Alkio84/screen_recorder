...
void ScreenRecorder::configureAudioInput() {
    ...
    av_dict_set(&audioOptions, "ac", "2", 0);
    av_dict_set(&audioOptions, "i", "hw:0", 0);
    if(avformat_open_input(&inputAudioFormatContext,"",inputFormat, &audioOptions) != 0)
...
void ScreenRecorder::CaptureAudio() {
    ...
    while (!end) {
        ...
        */
        ...
        if (av_audio_fifo_space(fifo) < inputFrame->nb_samples) throw std::runtime_error("Audio"
                                                                                         " buffer is too small.");
